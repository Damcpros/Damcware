...
NoAnchor = GuiLibrary.Windows.Movement.CreateButtonWithOptions({
	Name = "NoAnchor",
	Function = function(callback) 
		if callback then
			task.spawn(function()
				...
				until not NoAnchor.Enabled
			end)
		end
	end,
})

PlayerTp = GuiLibrary.Windows.Movement.CreateButtonWithOptions({
	Name = "PlayerTp",
	Function = function(callback) 
		if callback then
			task.spawn(function()
				repeat task.wait()
					pcall(function()
						lplr.Character.PrimaryPart.CFrame = CFrame.new(getNearestPlayerToMouse().Character.PrimaryPart.Position + Vector3.new(0,5,0))
					end)
